...
class FusedNavigator(context: Context) : Navigator(context) {
    ...
    private fun calculateHeading(): Float {
        ...
        if (success) {
            ...
            return ((Math.toDegrees(SensorManager.getOrientation(rMat, orientation)[0].toDouble()) + 360) % 360).toFloat()
        }
        return -1F
    }
